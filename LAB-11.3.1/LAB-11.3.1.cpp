...
void SortBIN(Student* p, const int N, char* fname, const int i, const int j);
...
void fWrite(fstream& f, const int i, const string x)
{
	f.seekp(i * (long)sizeof(string)); // встановили вказівник
	f.write((char*)&x, sizeof(string)); // записали значення
}

string fRead(fstream& f, const int i)
{
	string x;
	f.seekg(i * (long)sizeof(string)); // встановили вказівник
	f.read((char*)&x, sizeof(string)); // прочитали значення
	return x;
}

void SortBIN(Student* p, const int N, char* fname, const int i, const int j)
{
	fstream f(fname, ios::binary);		// відкрили файл для зчитування
	Student tmp;
